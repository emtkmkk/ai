...
ゲーム開始時（`start()`）に抽選されるオプション（巨大数・Max1・無限・2番目/中央値勝利・午前中長時間・フォロワー限定・公開限定）について、**前提条件を満たして抽選したが外れた場合**、次回以降の当選率が上がる。

- 連続外れ n 回のとき、実効確率は `1 - (1 - p) ^ (n + 1)`（n+1 回ロールのいずれか当選と等価）
- 当選すると streak は 0 にリセット
- 前提条件を満たさない場合は抽選も streak 更新も行わない
- 集計時の反転モード等は対象外
