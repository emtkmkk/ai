...
export default class extends Module {
	...
	public install() {
		...
		setInterval(() => {
			...
			const rnd =
				(hours === 12 || (hours > 17 && hours < 24) ? 0.5 : 0.1) *
	...
	private async start(triggerUserId?, flg?) {
		...
		if (this.ai.activeFactor >= 0.85) {
			...
			visibility =
				Math.random() < 0.03 && !triggerUserId ? "followers" : undefined;

			if (!visibility) {
				// 投稿がフォロワー限定でない場合は、3%の確率で公開投稿のみ受付けるモードにする
				publicOnly =
					this.ai.activeFactor >= 0.5 &&
					!recentGame?.publicOnly &&
					(recentGame?.publicOnly == null || Math.random() < 0.005);
			}
		}
		...
		// 90% → 5分 or 10分
		let limitMinutes =
			Math.random() < 0.1 && this.ai.activeFactor >= 0.75
				? Math.random() < 0.5 && !triggerUserId
					? 1
					: 2
				: Math.random() < 0.5
				? 5
			...
			limitMinutes =
				Math.floor(
					((1 /
						(1 -
							Math.min(
								(1 - this.ai.activeFactor) * 1.2 * (0.7 + Math.random() * 0.3),
								0.8
							))) *
						limitMinutes) /
						5
		...
		const post = await this.ai.post({
			text: !publicOnly
				...
				: serifs.kazutori.introPublicOnly(maxnum, limitMinutes, winRank),
	...
	private async mentionHook(msg: Message) {
		...
		if (recentGame) {
			...
			if (h > 0 && h < 8) {
				msg.reply(
					"いまは数取りをやるには夜遅すぎるからだめといわれてる時間なのじゃ 8時から開催を受け付けるのじゃ"
				);
				return {
			...
			if (
				...
				Date.now() - recentGame.startedAt < 1000 * 60 * 60
			) {
				const ct = Math.ceil(
					60 - (Date.now() - recentGame.startedAt) / (1000 * 60)
				);
		...
		msg
			.reply(
				"\n分かったのじゃ！数取りを開催するのじゃ！\nそなたは開催1分後から数取りに投票で参加することができるのじゃ！\n（こっそりだいれくとめっせーじなら今すぐでも大丈夫なのじゃ！）",
	...
	private async contextHook(key: any, msg: Message) {
		...
		if (
			game.triggerUserId === msg.user.id &&
			time < 60 * 1000 &&
			msg.visibility !== "specified"
		) {
			msg
				.reply(
					`\n${60 - Math.floor(time / 1000)}秒後にもう一度送ってほしいのじゃ！`,
					{ visibility: "specified" }
				)
				.then((reply) => {
		...
		if (
			game.publicOnly &&
			((msg.visibility != "public" && msg.visibility != "home") ||
				msg.localOnly)
		) {
			const visibility =
			...
			msg
				.reply(
					`\n公開投稿限定縛りなのじゃ！\n参加するには${
						visibility ? "「" + visibility + "」じゃなく、" : ""
					}「公開」または「ホーム」の公開範囲にてりぷらいしてほしいのじゃ～`
				)
				.then((reply) => {
					game.replyKey.push(msg.userId);
					this.games.update(game);
		...
		if (game.votes.some((x) => x.user.id == msg.userId)) {
			msg.reply("すでに投票済みの様じゃ").then((reply) => {
				game.replyKey.push(msg.userId);
				this.games.update(game);
				this.subscribeReply(msg.userId, reply.id);
			});
		...
		if (!msg.extractedText.includes("∞")) {
			...
			const match = msg.extractedText
				...
				.match(/[0-9]+/);
			if (match == null) {
				msg.reply("りぷらいの中に数字がないようにみえるが…").then((reply) => {
					game.replyKey.push(msg.userId);
					this.games.update(game);
					this.subscribeReply(msg.userId, reply.id);
				});
	...
	private crawleGameEnd() {
		...
		// 制限時間が経過していたら
		if (
			Date.now() - (game.finishedAt ?? game.startedAt + 1000 * 60 * 10) >=
			0
		) {
			this.finish(game);
		}
	...
	private finish(game: Game) {
		...
		const medal =
			game.votes?.length > 1 &&
			game.votes?.filter((x) => x.user.winCount < 50).length <
				game.votes?.filter((x) => x.user.winCount >= 50).length;
		...
		if (game.votes?.filter((x) => x.user.winCount < 50).length <= 1 && !medal) {
			...
			this.ai.decActiveFactor(
				((game.finishedAt.valueOf() - game.startedAt.valueOf()) /
					(60 * 1000 * 100)) *
					Math.max(1 - game.votes.length / 3, 0)
			);
		...
		if (!medal) {
			...
			if (
				...
				winner &&
				winDiff > 10 &&
				Math.random() < Math.min((winDiff - 10) * 0.02, 0.7)
			) {
				reverse = !reverse;
			} else if (
				reverse &&
