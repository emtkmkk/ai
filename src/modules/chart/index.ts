...
export default class extends Module {
    ...
    if (type === 'userNotes') {
      ...
      chart = {
        ...
        datasets: [
          ...
          {
            data: data.local.total.map(
              (x, i) => x - data.local.inc[i] - data.read[i],
            ),
          },
          {
            data: data.local.inc,
  ...
  private async mentionHook(msg: Message) {
    ...
    let title =
      type === 'random'
        ? /チャート\s?(\S{1,25})/.exec(msg.extractedText)?.[1]
        : undefined;
    ...
    this.log('Replying...');
    msg.reply(serifs.chart.foryou, {
      file,
      visibility: 'public',
      localOnly: !!msg.user.host && !!title,
    });
