...
1. **隅を取る**: 合法手のうち隅に打てるなら、反転数最大の手を選ぶ（複数隅候補はタイブレーク）。
2. **角周辺回避（条件付き）**: 空き角があるとき、Nearset（X/C/Line2）に含まれる手を避ける。すべて角周辺しかない場合は全合法手を候補に。
3. **X より C を優先**: 角周辺しか打てないとき、C（隅の横／縦隣）を優先、なければ X（隅の斜め隣）を候補にする。
4. **反転数最大化**: 候補のうち反転数が最大の手を選ぶ。

角・C/X は固定座標を使わず、盤面形状から計算する（変則盤対応）。
...
- **隅攻防**: put/undo で相手に隅を渡す手を除外。相手の隅アクセスを潰す手を優先。
- **優先**: 角 → 取得済み角の隣辺 → GoodEdge2 → GoodInner の順で、当たった時点で候補を絞りタイブレークへ。
- **追加回避**: 優先が一度も当たらなかったとき、空き角近く（Nearset）と辺から 1 マス離れた手を回避。
- **タイブレーク**: 空きマス数に応じて反転数 min/min+1 または max → 相手モビリティ最小（中盤以降）→ 全体フロンティア最小 → 返せる石の周りの空白が少ない手 → 8 近傍の空きが少ない手 → パリティ（空き ≤ 10 かつ 1 領域）→ 反転数最小（序盤のみ）→ 辺から遠い手 → ランダム。
...
2. **終局結果・難易度ボーナス倍率 + 盤面充填率補正**
   - モード倍率: 単純モード `×1.35` / それ以外 `×1.0`
   - 結果倍率: `iLose`（ユーザ勝ち / bot負け）`×2.0`、`drawn`（引き分け）`×1.5`、`iWon` / `youSurrendered` / `timeout` / `decline` は `×1.0`
   - 盤面充填率補正: 終局時に `総石数 < 全マス数` のとき、`×(全マス数 / 総石数)` を適用。
   - 盤面情報がない場合、`総石数 <= 0`、または `総石数 >= 全マス数` の場合は補正 `×1.0` とする。
   - 最終倍率は「モード倍率 × 結果倍率 × 盤面充填率補正」で計算する。
3. **チャンク化して加算**
   - `調整後思考時間（基礎時間 × 上記の最終倍率） + 同日内の既存繰越(loveThinkingCarryMs)` を 40 秒単位で区切り、チャンク数を計算。
