...
```mermaid
flowchart TD
    A["メンション受信"] --> B{"「ですか？」<br/>「ますか？」<br/>「ですよね？」<br/>で終わる?"}
    B -- はい --> C["YES/NO 回答"]
    B -- いいえ --> D{"返信先あり +<br/>「ファクトチェック」?"}
    D -- はい --> E["ファクトチェック"]
    D -- いいえ --> F{"NdM 形式?"}
    F -- はい --> G["サイコロ"]
    F -- いいえ --> H["スキップ"]
...
```mermaid
flowchart TD
    A["「ファクトチェック」を<br/>含むメンション"] --> B{"返信先が<br/>リモートユーザーの投稿?"}
    B -- はい --> C{"フォロー関係<br/>あり?"}
    C -- いいえ --> D["拒否<br/>「フォローしていない<br/>リモートユーザには…」"]
    C -- はい --> E
    B -- いいえ --> E{"返信先が<br/>藍自身の投稿?"}
    E -- はい --> F["「もちろん正しいです！」"]
    E -- いいえ --> G["判定結果を返信"]
```
