...
```mermaid
flowchart TD
    A["serverStats ストリーム<br/>から1秒ごとに統計受信"] --> B["最新30件のログを保持<br/>(0.5分間分)"]
    B --> C["3秒ごとに<br/>平均使用率を計算"]
    C --> D{"CPU ≥ 90%<br/>or メモリ ≥ 92%?"}
    D -- はい --> E{"警告済み?"}
    E -- はい --> Z["スキップ<br/>(連続警告防止)"]
    E -- いいえ --> F{"前回警告から<br/>1時間以内?"}
    F -- はい --> Z
    F -- いいえ --> G["管理者にDMで警告"]
    G --> H["警告済みフラグON"]
    D -- いいえ --> I{"CPU ≤ 30%<br/>かつ メモリ ≤ 60%?"}
    I -- はい --> J["警告済みフラグOFF<br/>(負荷回復)"]
    I -- いいえ --> K["待機"]
```
