...
export default class extends Module {
  ...
  private async linkAccountListAdd() {
    ...
    if (!this.list) {
      this.list = (await this.ai.api('users/lists/create', {
        name: 'Linked',
      })) as List;
      if (!this.list) return;
    ...
    if (this.list) {
      ...
      for (const linkedUser of linkedUsers) {
        if (
          ...
          Array.from(new Set(linkedUser.linkedAccounts))
        ) {
          linkedUser.linkedAccounts = Array.from(
  ...
  private getLove(msg: Message): boolean {
    if (!msg.text) return false;
    if (
      !msg.text.includes('好感度') &&
      !msg.text.includes('懐き度') &&
    ...
    const rpg = msg.friend.doc.perModulesData?.rpg
      ? [
          ...
          msg.friend.doc.perModulesData.rpg.skills
            ? `  ${serifs.rpg.status.skill} : `
            : undefined,
          ...(msg.friend.doc.perModulesData.rpg.skills
            ? msg.friend.doc.perModulesData.rpg.skills.map(
                (x) => '    ' + x.name,
              )
            : []),
          msg.friend.doc.perModulesData.rpg.coin
            ? `  ${serifs.rpg.status.coin} : ${msg.friend.doc.perModulesData.rpg.coin}`
            : '',
        ]
          .filter(Boolean)
          .join('\n')
