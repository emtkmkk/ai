...
export default class extends Module {
  ...
  private async mentionHook(msg: Message) {
    if (msg.includes(['迷路'])) {
      ...
      if (
        (msg.includes(['よっぱらい', '酔っぱらい']) &&
          msg.includes(['接待', '超かんたん'])) ||
        msg.includes(['超接待', '超超かんたん'])
      )
        size = 'veryVeryEasy';
      if (
        ...
        size = 'hard';
      if (msg.includes(['死', '鬼', '地獄', '超むずかしい', 'おに']))
        size = 'veryHard';
      if (
        (msg.includes(['スペシャル']) && msg.includes(['本気'])) ||
        (msg.includes(['裏']) && msg.includes(['おに']))
      )
        size = 'ai';
