...
export const shop2Items: ShopItem[] = [
	...
	{ name: `平常心のお札`, limit: (data) => !data.items.filter((x) => x.name === "平常心のお札").length && !data.bankItems?.filter((x) => x.name === "平常心のお札").length, price: 50, desc: `どれだけ体力が低くても決死の覚悟をしなくなる（ただしレイド時の6ターン目を除く）`, type: "token", effect: { notLastPower: true }, always: true } as TokenItem,
	{ name: `全身全霊のお札`, limit: (data) => !data.items.filter((x) => x.name === "全身全霊のお札").length && !data.bankItems?.filter((x) => x.name === "全身全霊のお札").length, price: 300, desc: `持っていると常に行動回数が1回になるが、すごく重い一撃を放てる`, type: "token", effect: { allForOne: true }, always: true } as TokenItem,
	{ name: `運命不変のお札`, limit: (data) => !data.items.filter((x) => x.name === "運命不変のお札").length && !data.bankItems?.filter((x) => x.name === "運命不変のお札").length, price: 300, desc: `持っていると常に与ダメージがランダム変化しなくなる`, type: "token", effect: { notRandom: true }, always: true } as TokenItem,
	{ name: `しあわせのお札`, limit: (data) => !data.items.filter((x) => x.name === "しあわせのお札").length && !data.bankItems?.filter((x) => x.name === "しあわせのお札").length, price: 300, desc: `レイド時、常にステータスの割合がランダムに一時的に変化する`, type: "token", effect: { fortuneEffect: true }, always: true } as TokenItem,
	{ name: `平常心のお札`, limit: (data) => !data.items.filter((x) => x.name === "平常心のお札").length && data.bankItems?.filter((x) => x.name === "平常心のお札").length, price: 1, desc: `どれだけ体力が低くても決死の覚悟をしなくなる（ただしレイド時の6ターン目を除く）`, type: "token", effect: { notLastPower: true }, always: true } as TokenItem,
	{ name: `全身全霊のお札`, limit: (data) => !data.items.filter((x) => x.name === "全身全霊のお札").length && data.bankItems?.filter((x) => x.name === "全身全霊のお札").length, price: 1, desc: `持っていると常に行動回数が1回になるが、すごく重い一撃を放てる`, type: "token", effect: { allForOne: true }, always: true } as TokenItem,
	{ name: `運命不変のお札`, limit: (data) => !data.items.filter((x) => x.name === "運命不変のお札").length && data.bankItems?.filter((x) => x.name === "運命不変のお札").length, price: 1, desc: `持っていると常に与ダメージがランダム変化しなくなる`, type: "token", effect: { notRandom: true }, always: true } as TokenItem,
	{ name: `しあわせのお札`, limit: (data) => !data.items.filter((x) => x.name === "しあわせのお札").length && data.bankItems?.filter((x) => x.name === "しあわせのお札").length, price: 1, desc: `レイド時、常にステータスの割合がランダムに一時的に変化する`, type: "token", effect: { fortuneEffect: true }, always: true } as TokenItem,
	{ name: `乱数透視の札`, limit: (data) => !data.items.filter((x) => x.name === "乱数透視の札").length && data.bankItems?.filter((x) => x.name === "乱数透視の札").length, price: 1, desc: `所持している間、ダメージの乱数が表示されるようになります`, type: "token", effect: { showRandom: true }, always: true } as TokenItem,
