...
export const skillPrice = (_ai: 藍, skillName: Skill["name"], rnd: () => number) => {
	...
	const price = Math.max(
		Math.floor(
			((skill?.unique ? 18 : 12) * (skill?.notLearn ? 2.5 : (Math.max(isNaN(skillP.skillNameCount) ? 0 : skillP.skillNameCount, 0.5) / (totalSkillCount / filteredSkills.filter((x) => !x.notLearn).length)))) ** 2
		), 100
	);
...
export const shop2Items: ShopItem[] = [
	...
	{ name: `行動加速のお札`, limit: (data) => (data.maxSpd ?? 0) < 5 && !data.items.filter((x) => x.name === "行動加速のお札").length, price: (data) => 370 - (data.maxSpd ?? 0) * 70, desc: `${config.rpgHeroName}が最低5回行動できるようになります`, type: "token", effect: { fivespd: true }, always: true } as TokenItem,
	ultimateAmulet,
