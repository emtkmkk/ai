...
export const shop2Items: ShopItem[] = [
	...
	{ name: `平常心のお札`, limit: (data) => !data.items.filter((x) => x.name === "平常心のお札").length && !data.bankItems?.filter((x) => x === "平常心のお札").length, price: 50, desc: `どれだけ体力が低くても決死の覚悟をしなくなる（ただしレイド時の6ターン目を除く）`, type: "token", effect: { notLastPower: true }, always: true } as TokenItem,
	...
	{ name: `平常心のお札`, limit: (data) => !data.items.filter((x) => x.name === "平常心のお札").length && data.bankItems?.filter((x) => x === "平常心のお札").length, price: 1, desc: `どれだけ体力が低くても決死の覚悟をしなくなる（ただしレイド時の6ターン目を除く）`, type: "token", effect: { notLastPower: true }, always: true } as TokenItem,
