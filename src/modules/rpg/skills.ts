...
export const skills: Skill[] = [
	...
	{ name: `${serifs.rpg.status.def}アップ`, short: `Ｄ`, desc: `常に${serifs.rpg.status.def}が上がります`, info: `${serifs.rpg.status.atk}+4% ${serifs.rpg.status.def}+13%`, effect: { atkUp7: 0.04, defUp: 0.13 } },
	...
	{ name: `疲れにくい`, short: "疲", desc: `疲れでダメージを受ける際にそのダメージを軽減します`, info: `ダメージメッセージに疲が入っている場合、${serifs.rpg.status.def}+27% ${serifs.rpg.status.atk}+4%`, effect: { atkUp7: 0.04, notBattleBonusDef: 0.27 } },
	{ name: `油断しない`, short: "断", desc: `ターン1に受けるダメージを大きく軽減します`, info: `ターン1にてダメージカット40%を得る\n100%以上になる場合、残りはターン2に持ち越す\n${serifs.rpg.status.atk}+4%`, effect: { atkUp7: 0.04, firstTurnResist: 0.4 }, skillOnly: true },
	{ name: `粘り強い`, short: "粘", desc: `体力が減るほど受けるダメージを軽減します`, info: `ダメージカット25%×(減少HP割合)を得る 最大90% ${serifs.rpg.status.atk}+4%`, effect: { atkUp7: 0.04, tenacious: 0.25 } },
	{ name: `高速RPG`, short: "速", desc: `1回のRPGでお互いに2回行動します`, info: `1回のコマンドで2ターン進行する ${serifs.rpg.status.atk}+1% レイド時は、さらに${serifs.rpg.status.atk}+10%`, effect: { atkUp7: 0.01, plusActionX: 1 } },
	...
	{ name: `${serifs.rpg.dmg.take}${serifs.rpg.status.rndM}`, short: "安", desc: `敵から受ける最大ダメージを減少させます`, info: `敵の乱数幅 20~180 -> 20~160 (%) ${serifs.rpg.status.atk}+4% 乱数系と重複しない`, effect: { atkUp7: 0.04, defRndMin: 0, defRndMax: -0.2 }, unique: "rnd" },
	{ name: `${serifs.rpg.dmg.take}${serifs.rpg.status.rndP}`, short: "不", desc: `敵から受ける最小ダメージを減少させます`, info: `敵の乱数幅 20~180 -> 0~180 (%) ${serifs.rpg.status.atk}+4% 乱数系と重複しない`, effect: { atkUp7: 0.04, defRndMin: -0.2, defRndMax: 0 }, unique: "rnd" },
	...
	{ name: `防具が大好き`, short: "防", desc: `防具を装備しやすくなり、防具の効果量が上がります`, info: `防具装備率3倍 防具効果量+70% ${serifs.rpg.status.atk}+4% 種類大好き系と重複しない`, effect: { atkUp7: 0.04, armorSelect: 2, armorBoost: 0.7 }, unique: "itemSelect" },
	{ name: `食いしんぼう`, short: "食", desc: `食べ物を食べやすくなり、食べ物の効果量が上がります`, info: `食べ物使用率3倍 食べ物効果量+70% 毒食べ物ダメージ-60% ${serifs.rpg.status.atk}+4% 種類大好き系と重複しない`, effect: { atkUp7: 0.04, foodSelect: 2, foodBoost: 0.7, poisonResist: 0.6 }, unique: "itemSelect" },
	...
	{ name: `お腹が空いてから食べる`, short: "空", desc: `体力が減ったら食べ物を食べやすくなり、食べ物の効果量が少し上がります`, info: `体力が減少すれば食べ物を食べるようになり、毒食べ物の確率が下がる 食べ物効果量+20% 毒食べ物ダメージ-20% ${serifs.rpg.status.atk}+4% `, effect: { atkUp7: 0.04, lowHpFood: 1, foodBoost: 0.2, poisonResist: 0.2 }, unique: "lowHpFood" },
	...
	{ name: `敵のクリティカル性能減少`, short: "守", desc: `相手のクリティカル率とクリティカルダメージが減少します`, info: `敵のクリティカル率-40% 敵のクリティカルダメージ-40% ${serifs.rpg.status.atk}+4% レイド時は、追加で${serifs.rpg.status.def}+10%`, effect: { atkUp7: 0.04, enemyCritDown: 0.4, enemyCritDmgDown: 0.4 } },
...
export function aggregateSkillsEffects(data: any): SkillEffect {
	...
	aggregatedEffect.atkUp = (1 + (aggregatedEffect.atkUp ?? 0)) * (1 + (aggregatedEffect.atkUp2 ?? 0)) * (1 + (aggregatedEffect.atkUp3 ?? 0)) * (1 + (aggregatedEffect.atkUp4 ?? 0)) * (1 + (aggregatedEffect.atkUp5 ?? 0)) * (1 + (aggregatedEffect.atkUp6 ?? 0)) * (1 + (aggregatedEffect.atkUp7 ?? 0));
...
export function aggregateSkillsEffectsSkillX(data: any, skillX: number): SkillEffect {
	...
	aggregatedEffect.atkUp = (1 + (aggregatedEffect.atkUp ?? 0)) * (1 + (aggregatedEffect.atkUp2 ?? 0)) * (1 + (aggregatedEffect.atkUp3 ?? 0)) * (1 + (aggregatedEffect.atkUp4 ?? 0)) * (1 + (aggregatedEffect.atkUp5 ?? 0)) * (1 + (aggregatedEffect.atkUp6 ?? 0)) * (1 + (aggregatedEffect.atkUp7 ?? 0)) * uniqueX;
