  ...
  const friends = ai.friends
    ...
    .filter(
      (x) =>
        ...
        x.perModulesData.rpg.skills?.length,
    );
      ...
      skills.forEach((skill) => {
        ...
        if (skillName) {
          ...
          if (skillNameCountMap.has(skillName)) {
            skillNameCountMap.set(
              skillName,
              skillNameCountMap.get(skillName) + 1,
            );
          } else {
...
export const skills: Skill[] = [
  ...
  },
  {
    name: `テキパキこなす`,
    short: '効',
    desc: `戦闘以外の事の効率が上がります`,
    info: `非戦闘時、${serifs.rpg.status.atk}+20%`,
    effect: { notBattleBonusAtk: 0.2 },
  },
  {
    name: `疲れにくい`,
    short: '疲',
    desc: `疲れでダメージを受ける際にそのダメージを軽減します`,
    info: `ダメージメッセージに疲が入っている場合、${serifs.rpg.status.def}+18%`,
  ...
  },
  {
    ...
    info: `ダメージカット20%×(減少HP割合)を得る 最大90%`,
    effect: { tenacious: 0.2 },
  },
  {
    name: `高速RPG`,
    short: '速',
  ...
  },
  {
    name: `1時間先取りRPG`,
    short: '先',
    desc: `1時間早くRPGをプレイする事が出来ます`,
    info: `1時間早くRPGプレイ可能 ステータス+5%`,
  ...
  },
  {
    name: `強敵と戦うのが好き`,
    short: '強',
    desc: `敵が強ければステータスが上昇します`,
    info: `ステータス+(敵の攻撃 × 敵の防御 / 4)%`,
    effect: { enemyStatusBonus: 1 },
  },
  {
  ...
  },
  {
    ...
    short: '乱',
    desc: `乱数幅が20~180 -> 5~230になります クリティカル率も上がります`,
    info: `乱数幅 20~180 -> 5~230 (%) 期待値 115% クリティカル率+2% 乱数系と重複しない`,
  ...
  },
  {
    ...
    info: `ターン1装備率+100% ターン1悪アイテム率-100% アイテム装備率-15% 重複しない`,
    effect: { firstTurnItem: 1, firstTurnMindMinusAvoid: 1, itemEquip: -0.15 },
  ...
  },
  {
    ...
    desc: `道具の使用率が上がります`,
    info: `アイテム装備率+40%`,
    effect: { itemEquip: 0.4 },
  },
  {
    name: `道具の扱いが上手い`,
    short: '扱',
    desc: `道具の効果量が上がります`,
    info: `アイテム効果量+40% アイテム悪効果軽減+40%`,
    effect: { itemBoost: 0.4 },
  },
  {
    name: `武器が大好き`,
    short: '武',
  ...
  },
  {
    name: `なんでも口に入れない`,
    short: '捨',
    desc: `良くないものを食べなくなることがあります`,
    info: `毒食べ物を50%で捨てる 100%以上になると悪アイテム率減少に変換`,
    effect: { poisonAvoid: 0.5 },
  },
  {
    name: `道具の選択が上手い`,
    short: '選',
    desc: `道具の効果量がすこし上がり、悪いアイテムを選びにくくなります`,
    info: `道具効果量+15% 悪アイテム率-15%`,
  ...
  },
  {
    name: `たまにたくさん成長`,
    short: '成',
    desc: `たまにステータスが多く増加します ★変更不可`,
    info: `2Lv毎になにかのステータス+1 ★変更不可（変更してもステータスは残るため）`,
  ...
  },
  {
    name: `クリティカル性能上昇`,
    short: '急',
    desc: `クリティカル率とクリティカルダメージが上昇します`,
    info: `クリティカル率1.2倍&+3% クリティカルダメージ+20%`,
  ...
  },
  {
    ...
    info: `敵のクリティカル率-30% 敵のクリティカルダメージ-30% レイド時は、${serifs.rpg.status.def}+10%`,
    effect: { enemyCritDown: 0.3, enemyCritDmgDown: 0.3 },
  },
  {
    name: `負けた時、しっかり反省`,
    short: '省',
    desc: `敗北時のボーナスが上昇します ★変更不可`,
    info: `敗北毎にステータス+2 ★変更不可（変更してもステータスは残るため）`,
    effect: { loseBonus: 1 },
    unique: 'loseBonus',
    cantReroll: true,
  },
...
export const skillReply = (module: Module, ai: 藍, msg: Message) => {
  ...
  if (
    ...
    if (!data.duplicationOrb || data.duplicationOrb <= 0)
        ...
        if (!playerSkills[i].cantReroll) {
          ...
          msg.reply(
            `\n` +
              ...
              (aggregateTokensEffects(data).showSkillBonus &&
              data.skills[i].info
  ...
  msg.reply(
    [
      data.rerollOrb && data.rerollOrb > 0
        ? serifs.rpg.skills.info(data.rerollOrb) + '\n'
        : '',
      data.duplicationOrb && data.duplicationOrb > 0
        ? serifs.rpg.skills.duplicationInfo(data.duplicationOrb) + '\n'
        : '',
 ...
 */
export function aggregateSkillsEffects(data: {
  items?: ShopItem[];
  skills: Skill[];
}): SkillEffect {
  ...
  if (aggregatedEffect.poisonAvoid && aggregatedEffect.poisonAvoid > 1) {
    aggregatedEffect.mindMinusAvoid =
      ...
      (aggregatedEffect.poisonAvoid - 1) * 0.6;
  ...
  if (aggregatedEffect.abortDown && aggregatedEffect.abortDown > 1) {
    aggregatedEffect.atkUp =
      ...
      (aggregatedEffect.abortDown - 1) * (1 / 3);
  ...
  if (aggregatedEffect.enemyCritDown && aggregatedEffect.enemyCritDown > 1) {
    aggregatedEffect.defUp =
      ...
      (aggregatedEffect.enemyCritDown - 1) * (1 / 3);
...
export function getSkillsShortName(data: {
  items?: ShopItem[];
  skills: Skill[];
}): { skills?: string | undefined; amulet?: string | undefined } {
  const dataSkills = data.skills?.length
    ? '[' +
      data.skills
        .map((x) => (skills.find((y) => x.name === y.name) ?? x).short)
        .join('') +
      ']'
    : undefined;
  const amulet = data.items?.filter((x) => x.type === 'amulet').length
    ? (data.items?.filter((x) => x.type === 'amulet')[0] as AmuletItem)
    : undefined;
  const amuletItem = amulet
    ...
    : undefined;
  const amuletShort = amuletItem?.short
    ? '[' + amuletItem.short + ']'
    : undefined;
