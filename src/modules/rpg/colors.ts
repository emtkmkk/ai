...
export const colors: Color[] = [
		...
		message: (data) =>
			data.allClear
				...
				: `連勝で全ての敵を倒すと解放されちゃうのじゃ！ふっふーん！${
						data.clearEnemy?.length ? `(現在 **${data.clearEnemy.length}** 連勝中)` : ''
				  }`,
	},
	{
		id: 5,
...

	{
		...
		unlock: (data) =>
			...
			(data.superCount ?? 0) >= Math.ceil((100 * (7 - unlockCount(data, [9], true))) / 7) - unlockCount(data, [9], true),
		message: (data) =>
			unlockCount(data, [9]) >= 8 ||
			(data.superCount ?? 0) >= Math.ceil((100 * (7 - unlockCount(data, [9], true))) / 7) - unlockCount(data, [9], true)
				? `${serifs.rpg.color.unlock} (気合耐え発動率: **${10 + (data.endure ?? 0) * 5}** %)`
				: `色を8種類解放する、または${
						Math.ceil((100 * (7 - unlockCount(data, [9], true))) / 7) - unlockCount(data, [9], true)
...
export const unlockCount = (data, excludeIds: number[] = [], excludeDefault = false) => {
	return (
		excludeDefault
			? colors.filter((x) => !excludeIds.includes(x.id)).filter((x) => !x.default)
