...
プレイ枠を使わずに終わった場合、その枠ごとに **活力** が1溜まる（maxLv 到達後も対象）。蓄積は **遅延計算** で、RPGプレイ時やステータス表示時に `lastVitalitySlot` から終了済み枠を遡って加算する（全ユーザーの定時処理は行わない）。初回計算時は Lv1 未プレイ以外は `lastPlayedAt`（最終プレイ枠）から遡及する。`vitalityBackfilled` 未設定の既存ユーザーは次回の活力計算時に一度だけリセットしてから遡及する。活力は Lv には影響せず、通常RPG・おかわりプレイ時に次のように消費される。

| 効果 | 内容 |
|------|------|
| 消費上限 | 1プレイあたり最大20 |
| 投稿数 | 消費1につき +15 |
| もこコイン | 消費量と同数をそのプレイで引き出し（5枚報酬と同時の場合は表示枚数を合算） |

毎日プレイすれば活力は溜まらない。レイド・木人・旅モードでは消費しない。

### レイド活力（レイド未参加の救済）

過去に1回以上レイドに参加したことがあるユーザー向け。通常の活力とは**別フィールド**。

| 項目 | 内容 |
|------|------|
| 蓄積 | 終了したレイドに未参加のたび +1（遅延計算。ステータス表示・レイド参加時） |
| 蓄積対象 | `lastRaidParticipatedPostId` があるユーザー（初回レイド参加者は参加まで蓄積しない） |
| 導入時 | 既存参加者は履歴から `postId` を埋める。導入前のスキップは数えず、導入後の未参加のみ +1 |
| 消費 | レイド参加時。調子補正・HP補正の上限まで必要分だけ消費。戦闘中は条件を満たすターンごとに追加消費 |
| 調子補正 | `bonusX` +0.25/個、上限 1.0（ジャックポット前の base 部分） |
| HP補正 | ロール後、`bonusMaxHp` まで 25% ずつ引き上げ（`bonusMaxHp` を超えない） |
| 与ダメ上乗せ | 通常レイドのみ。投影個人評価が★5未満かつ活力≥1のターンで、活力1消費し当ターンの通常攻撃全ヒットに `floor(★3ノルマ/spd)` を加算（2以下なら消費しない）。戦士・スキル追加攻撃は対象外 |
| 鳩車レイド | 補正・消費なし（温存される） |
| じゃんけんレイド | HP 補正のみ（調子補正なし） |
...
- 投稿数を取得し、ステータス倍率 `tp` を算出
  - 活力があれば最大20まで消費し、投稿数に +15/活力 を加算
  - 連続ボーナスで投稿数にボーナス加算
