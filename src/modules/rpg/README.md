...
プレイ枠を使わずに終わった場合、その枠ごとに **活力** が1溜まる（maxLv 到達後も対象）。蓄積は **遅延計算** で、RPGプレイ時やステータス表示時に `lastVitalitySlot` から終了済み枠を遡って加算する（全ユーザーの定時処理は行わない）。活力は Lv には影響せず、通常RPG・おかわりプレイ時に次のように消費される。
