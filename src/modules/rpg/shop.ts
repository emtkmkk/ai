...
export const shopItems: ShopItem[] = [
	...
	{ name: `わかばのお守り`, limit: (data) => data.skills?.length < 4, price: (data) => Math.max(Math.min(data.coin, 10), 1), desc: `もこチキの持っているスキルが5個より少ない場合（もこチキのレベルが低い場合）、少ないスキル1つにつき約5%分パワー・防御が上がります 特定条件でさらにパワー・防御が+10%されます 耐久20`, type: "amulet", effect: { beginner: 0.05 }, durability: 20, short: "🔰", isUsed: (data) => true } as AmuletItem,
