...
export const shopItems: ShopItem[] = [
  ...
  },
  {
    name: '自動旅モードの札',
    limit: (data) =>
      !data.items.filter((x) => x.name === '自動旅モードの札').length,
    desc: '所持している間、旅モードに自動で突入します',
    price: 5,
    type: 'token',
  ...
  },
  {
    ...
    limit: (data) =>
      data.items.filter((x) => x.name === '自動旅モードの札').length,
    desc: '旅モードに自動で突入しなくなります',
    price: 0,
    type: 'item',
    effect: (data) =>
      (data.items = data.items.filter((x) => x.name !== '自動旅モードの札')),
    always: true,
  },
  {
    name: '乱数透視の札',
  ...
  },
  {
    ...
    type: 'item',
    effect: (data) => {
      data.atk = (data.atk ?? 0) + (data.def ?? 0) / 50;
      data.def = (data.def ?? 0) - (data.def ?? 0) / 50;
    },
    infinite: true,
  },
  {
  ...
  },
  {
    ...
    type: 'item',
    effect: (data) => {
      data.def = (data.def ?? 0) + (data.atk ?? 0) / 50;
      data.atk = (data.atk ?? 0) - (data.atk ?? 0) / 50;
    },
    infinite: true,
  },
  {
  ...
  },
  {
    name: `交通安全のお守り`,
    price: 30,
    desc: `持っているとターン1で30%ダメージカット 耐久6 使用時耐久減少`,
    type: 'amulet',
    effect: { firstTurnResist: 0.3 },
    durability: 6,
    isUsed: (data) => data.count === 1,
  },
  {
    name: `高級交通安全のお守り`,
    price: 70,
    desc: `持っているとターン1で60%ダメージカット 耐久6 使用時耐久減少`,
    type: 'amulet',
    effect: { firstTurnResist: 0.6 },
    durability: 6,
    isUsed: (data) => data.count === 1,
  },
  {
    ...
    desc: `購入時、気合アップ`,
    type: 'item',
    effect: (data) => (data.endure = (data.endure ?? 0) + 2),
  },
  {
    name: `壺`,
    price: 200,
  ...
  ...skills
    .filter(
      (x) =>
        !x.moveTo && !x.cantReroll && !x.unique && !x.effect.firstTurnResist,
    )
    .map(
      (x): AmuletItem => ({
        name: `${x.name}のお守り`,
...
export const shopReply = async (module: Module, msg: Message) => {
  ...
  const _shopItems = (data.shopItems as string[])
    .map((x) => shopItems.find((y) => x === y.name) ?? undefined)
  ...
  const reply = await msg.reply(
    [
      ...
      ...showShopItems.map(
        (x, index) =>
          `[${index + 1}] ${x.name} ${getVal(x.price, [data, rnd])}枚\n${x.desc}\n`,
      ),
    ].join('\n'),
    ...
    if (msg.includes([String(i + 1)])) {
      if (data.showShopItems[i].price <= rpgData.coin) {
        ...
        if (data.showShopItems[i].type === 'item') {
          ...
          if (!item) return { reaction: ':neofox_think:' };
          ...
          if (!item.infinite)
            rpgData.shopItems = rpgData.shopItems?.filter(
              (x) => data.showShopItems[i].name !== x.name,
            );
        } else {
