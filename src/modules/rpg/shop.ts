...
export const shopItems: ShopItem[] = [
    ...
    { name: `呪いの人形`, limit: (data) => data.revenge, price: 44, desc: `持っていると前回負けた敵に戦う際に20%ステータスアップ 耐久1 リベンジ成功時耐久減少`, type: "amulet", effect: { atkUp: 0.2, defUp: 0.2 }, durability: 1, isUsed: (data) => data.enemy?.name === data.revenge && data.ehp < 0, isMinusDurability: (data) => !data.revenge },
...
export const shopReply = async (module: Module, msg: Message) => {
    ...
    const filteredShopItems = shopItems.filter((x) => (!x.limit || x.limit(data, rnd)) && !(data.items.some((y) => y.type === "amulet") && x.type === "amulet") && !x.always)

    if (data.lastShopVisited !== getDate()) {
        data.shopItems = [
            filteredShopItems[rnd() * filteredShopItems.length].name,
            filteredShopItems[rnd() * filteredShopItems.length].name,
            filteredShopItems[rnd() * filteredShopItems.length].name,
            filteredShopItems[rnd() * filteredShopItems.length].name,
            filteredShopItems[rnd() * filteredShopItems.length].name
    ...
    const _shopItems = (data.shopItems as ShopItem[]).map((x) => shopItems.find((y) => x.name === y.name) ?? undefined).filter((x) => x != null);

    const showShopItems = _shopItems.filter((x) => (!x.limit || x.limit(data, rnd)) && !(data.items.some((y) => y.type === "amulet") && x.type === "amulet")).concat(filteredShopItems.filter((x) => x.always)).slice(0, 9)
    ...
    module.subscribeReply("shopBuy:" + msg.userId, reply.id, { showShopItems: showShopItems.map((x) => ({ name: x.name, type: x.type, price: getVal(x.price, [data, rnd]), ...(x.type === "amulet" ? {durability: x.durability ?? undefined, skillName: x.skillName } : {})}))});
    ...
    for (let i = 0; i < data.showShopItems.length; i++) {
        if (msg.includes([String(i + 1)])) {
            if (data.showShopItems[i].price >= rpgData.coin) {
                ...
                if (data.showShopItems.type === "item") {
                    const item = shopItems.find((x) => x.name === data.showShopItems.name) as Item
                    if (!item) return
                    const [_lv, _atk, _def] = [data.lv, data.atk, data.def];
                    item.effect(data);
                    const [lvDiff, atkDiff, defDiff] = [data.lv-_lv, data.atk-_atk, data.def-_def]
                    ...
                    if (lvDiff || atkDiff || defDiff) {
                        message += [
                            ...
                            `${serifs.rpg.status.lv} : ${data.lv ?? 1}${lvDiff !== 0 ? ` (${lvDiff > 0 ? "+" + lvDiff : lvDiff})` : ""}`,
                            `${serifs.rpg.status.atk} : ${data.atk ?? 0}${atkDiff !== 0 ? ` (${atkDiff > 0 ? "+" + atkDiff : atkDiff})` : ""}`,
                            `${serifs.rpg.status.def} : ${data.def ?? 0}${defDiff !== 0 ? ` (${defDiff > 0 ? "+" + defDiff : defDiff})` : ""}`,
                        ].filter(Boolean).join("\n")
                    }
                } else {
                    data.items.push(data.showShopItems[i])
                }
...
export function aggregateTokensEffects(data: { items: ShopItem[] }): any {
    ...
    data.items.forEach(_items => {
        ...
        const item = shopItems.find((x) => x.name === _items.name && x.type === "token") ?? _items;
