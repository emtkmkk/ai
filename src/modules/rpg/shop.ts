...
export const skillPrice = (
  ...
  const totalSkillCount = filteredSkills.reduce(
    (acc, skill) => acc + (skillP.skillNameCountMap.get(skill.name) || 0),
    0,
  );
...
export const shopItems: ShopItem[] = [
  ...
  },
  {
    ...
    limit: (data) =>
      (data.maxEndress ?? 0) > 0 &&
      !data.items.filter((x) => x.name === '自動修行モードの札').length,
    desc: '所持している間、修行モードに自動で突入します',
    price: (data) =>
      data.allClear ? 1 : Math.max(8 - (data.maxEndress ?? 0), 1),
    type: 'token',
    effect: { autoJournal: true },
    always: true,
  },
  {
    name: '穢根くんの身分証の写し',
  ...
  {
    name: 'おかわり2RPG自動支払いの札を捨てる',
    limit: (data) =>
      data.items.filter((x) => x.name === 'おかわり2RPG自動支払いの札').length,
    desc: 'おかわりおかわりRPGをプレイする際に毎回確認を表示します',
    price: 0,
    type: 'item',
    effect: (data) =>
      (data.items = data.items.filter(
        (x) => x.name !== 'おかわり2RPG自動支払いの札',
      )),
    always: true,
  },
  {
    ...
    limit: (data) =>
      data.items.filter((x) => x.name === '自動修行モードの札').length,
    desc: '修行モードに自動で突入しなくなります',
    price: 0,
    type: 'item',
    effect: (data) =>
      (data.items = data.items.filter((x) => x.name !== '自動修行モードの札')),
    always: true,
  },
  {
  ...
  },
  {
    ...
    limit: (data) =>
      data.lv >= 20 &&
      !data.items.filter((x) => x.name === '投稿数ボーナス表示の札').length,
    desc: '所持している間、投稿数ボーナスの詳細情報が表示されるようになります',
    price: 50,
    type: 'token',
    effect: { showPostBonus: true },
  },
  {
    name: 'スキル詳細表示の札',
    limit: (data) =>
      data.lv >= 20 &&
      !data.items.filter((x) => x.name === 'スキル詳細表示の札').length,
    desc: '所持している間、スキルの詳細情報が表示されるようになります',
    price: 50,
    type: 'token',
  ...
  },
  {
    ...
    type: 'item',
    effect: (data) => {
      data.atk = (data.atk ?? 0) + 1;
      data.def = (data.def ?? 0) - 1;
    },
    infinite: true,
  },
  {
  ...
  },
  {
    ...
    type: 'item',
    effect: (data) => {
      data.def = Math.round((data.def ?? 0) + Math.round((data.atk ?? 0) / 50));
      data.atk = Math.round((data.atk ?? 0) - Math.round((data.atk ?? 0) / 50));
    },
    infinite: true,
  },
  {
  ...
  {
    name: '仕切り直しの札',
    desc: '全ての敵が再出現するようになります',
    limit: (data, rnd) =>
      data.allClear && data.clearEnemy?.length && (data.maxEndress ?? 0) >= 6,
    price: 1,
    type: 'item',
    effect: (data) => (data.clearEnemy = []),
    always: true,
  },
  {
    name: 'しあわせ草',
  ...
  {
    name: 'タクシーチケット',
    desc: '購入時、修行モードのステージがベスト-1になる',
    limit: (data, rnd) => (data.maxEndress ?? 0) - (data.endress ?? 0) > 2,
    price: (data, rnd) =>
      ((data.maxEndress ?? 0) - (data.endress ?? 0) - 1) * 8,
    type: 'item',
    effect: (data) => {
      data.endress = (data.maxEndress ?? 0) - 1;
    },
    infinite: true,
  },
  {
  ...
  } as AmuletItem,
  {
    name: `交通安全のお守り`,
    price: 30,
    desc: `持っているとターン1で30%ダメージカット 耐久10 使用時耐久減少`,
    type: 'amulet',
  ...
  } as AmuletItem,
  {
    name: `高級交通安全のお守り`,
    price: 99,
    desc: `持っているとターン1で70%ダメージカット 耐久10 使用時耐久減少`,
    type: 'amulet',
  ...
  } as AmuletItem,
  {
    name: `全力の一撃のお守り`,
    price: 20,
    desc: `レイド時、ターン7で発生する全力の一撃を強化します 耐久10 レイドでの使用時耐久減少`,
    type: 'amulet',
...
export function mergeSkillAmulet(ai, rnd = Math.random, skills: Skill[]) {
  ...
  const price = Math.floor(
    priceSum *
      (Math.pow(1.5, uniqueSkills.length - 1) *
        Math.max(
          prices.reduce((pre, cur) => pre * (0.5 + cur / 24), 1),
          1,
        )),
  );
...
export const shopReply = async (module: rpg, ai: 藍, msg: Message) => {
  ...
  const _shopItems = (data.shopItems as (string | string[])[])
    .map((x) =>
      Array.isArray(x)
        ...
        : shopItems.find((y) => x === y.name) ?? undefined,
    )
    .filter((x) => x != null) as ShopItem[];

  const showShopItems = _shopItems
    .filter(
      (x) =>
        (!x.limit || x.limit(data, () => 0)) &&
  ...
  module.subscribeReply('shopBuy:' + msg.userId, reply.id, {
    showShopItems: showShopItems.map((x) => ({
      ...
      ...(x.type === 'amulet'
        ? {
            durability: x.durability ?? undefined,
            skillName: x.skillName ?? undefined,
          }
        : {}),
    })),
  });
...
export function shopContextHook(
  module: Module,
  key: any,
  msg: Message,
  data: any,
...
export function aggregateTokensEffects(data: { items: ShopItem[] }): any {
  ...
  data.items.forEach((_items) => {
    ...
    Object.entries(item.effect).forEach(([key, value]) => {
      if (
        typeof aggregatedEffect[key] === 'number' &&
        typeof value === 'number' &&
        aggregatedEffect[key] !== undefined
      ) {
