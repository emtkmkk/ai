...
export default class extends Module {
	...
	private async mentionHook(msg: Message) {
		...
		if (
			...
			// 各種データがない場合は、初期化
			if (!data.clearEnemy)
				data.clearEnemy = [data.preEnemy ?? ""].filter(Boolean);
		...
		if (msg.includes([serifs.rpg.command.rpg])) {
			...
			if (
				...
				if (
					...
					if (new Date().getHours() >= 18 && data.lastPlayedAt === getDate())
						continuousFlg = true;
					continuousBonus = 0.5;
				}
			}
			...
			let predictedDmg =
				Math.round(
					atk *
						...
						1.8 *
						(1 /
							((enemyDef * (this.getVal(data.enemy.defx, [tp]) ?? 3) + 100) /
								100))
				) * (abort || spd);
			...
			if (enemyHp <= 0) {
				...
				if (!enemyTurnFinished) {
					...
					if (
						...
						count === 1 &&
						Math.random() < 0.05 + 0.1 * (data.endure ?? 0)
					) {
						message += serifs.rpg.endure + "\n";
						playerHp = 1;
						data.endure = Math.max(data.endure - 1, 0);
				...
				if (playerHp <= 0) {
					...
					// 決着がつかない場合
					message +=
						this.showStatus(data, playerHp, enemyHp, enemyMaxHp, me) +
						"\n\n" +
						serifs.rpg.next;
			...
			while (
				data.lv >= 3 &&
				data.atk + data.def + totalUp < (data.lv - 1) * 7
			) {
				totalUp += 1;
				if (Math.random() < 0.5) atkUp += 1;
			}
			...
			if (data.atk > 0 && data.def > 0) {
				...
				if (Math.random() < (diff > 0 ? totalrate - rate : rate))
					atkUp = totalUp;
				else if (Math.random() < (diff < 0 ? totalrate - rate : rate))
					atkUp = 0;
			}
	...
	private showStatus(
		...
		// 敵
		const enemyHpMarkCount = Math.min(
			Math.ceil(enemyHp / enemyMaxHp / (1 / 7)),
			7
		);
		const enemyHpMarkStr = data.enemy.lToR
			? data.enemy.mark2.repeat(7 - enemyHpMarkCount) +
			  data.enemy.mark.repeat(enemyHpMarkCount)
			: data.enemy.mark2.repeat(enemyHpMarkCount) +
			  data.enemy.mark.repeat(7 - enemyHpMarkCount);
		const enemyHpInfoStr = data.enemy.lToR
			? Math.ceil(
					(100 - Math.min(Math.ceil(enemyHp / enemyMaxHp / (1 / 100)), 100)) / 5
			  ) *
					5 +
			  " " +
			  serifs.rpg.infoPercent
			: Math.ceil(
					Math.min(Math.ceil(enemyHp / enemyMaxHp / (1 / 100)), 100) / 5
			  ) *
					5 +
			  " " +
			  serifs.rpg.infoPercent;
		...
		const playerHpMarkStr = data.enemy.pLToR
			? serifs.rpg.player.mark2.repeat(7 - playerHpMarkCount) +
			  serifs.rpg.player.mark.repeat(playerHpMarkCount)
			: serifs.rpg.player.mark2.repeat(playerHpMarkCount) +
		...
		if (data.enemy.pLToR) {
			return `\n${data.enemy.hpmsg ? serifs.rpg.player.hpmsg : me} : ${
				data.info && (data.clearHistory ?? []).includes(data.enemy.name)
					? enemyHpInfoStr
					: enemyHpMarkStr
		...
		if (!chart?.diffs) {
			...
			if (msg.friend.doc.linkedAccounts?.length) {
				for (const userId of msg.friend.doc.linkedAccounts) {
					...
					if (
						!friend ||
						!friend.doc?.linkedAccounts?.includes(msg.friend.userId)
					)
						continue;
					...
					if (data.noChart && data.todayNotesCount) {
						postCount += Math.max(
							(friend.doc.user?.notesCount ?? data.todayNotesCount) -
								data.todayNotesCount,
							data.todayNotesCount -
								(data.yesterdayNotesCount ?? data.todayNotesCount)
						);
	...
	private getAtkDmg(
		...
		let dmg = Math.round(
			atk *
				...
				(crit ? 2 : 1) *
				(1 /
					((enemyDef * (defx ?? this.getVal(data.enemy.defx, [tp]) ?? 3) +
						100) /
						100))
		);
		if (data.fireAtk > 0) {
