...
export default class extends Module {
  ...
  private async mentionHook(msg: Message) {
    ...
    if (
      msg.includes([serifs.rpg.command.rpg]) &&
      msg.includes(
        Array.isArray(serifs.rpg.command.color)
          ? serifs.rpg.command.color
          : [serifs.rpg.command.color],
      )
    ) {
      // 色モード
    ...
    if (
      msg.includes([serifs.rpg.command.rpg]) &&
      msg.includes(
        Array.isArray(serifs.rpg.command.trial)
          ? serifs.rpg.command.trial
          : [serifs.rpg.command.trial],
      )
    ) {
      // 木人モード
  ...
  private remindLevelUpdate() {
    const filteredColors = colors
      .filter(
        (x) => x.id > 1 && !x.reverseStatus && !x.alwaysSuper && !x.hidden,
      )
      .map((x) => x.name);
    const me =
      Math.random() < 0.8
  ...
  private async scheduleDailyNoteCountsUpdate() {
    ...
    if (
      ...
      const friends = this.ai.friends
        .find()
        .filter(
          (x) =>
            x.perModulesData?.rpg?.lv &&
            x.perModulesData.rpg.lv > 1 &&
            x.perModulesData.rpg.noChart,
        );
  ...
  private handleRecordCommands(msg: Message) {
    ...
    if (data.jar) {
      message.push(
        createRankMessage(data.jar, 'チェキ購入数', 'jar', { suffix: '個' }),
      );
    }
  ...
  private async handleTrialCommands(msg: Message) {
    ...
    for (let i = 0; i < spd; i++) {
      ...
      let minDmg =
        getAtkDmg(data, atk, tp, 1, false, edef, 0, minRnd * buff, 3) + trueDmg;
      minTotalDmg += minDmg;
      let dmg =
        getAtkDmg(data, atk, tp, 1, false, edef, 0, rng * buff, 3) + trueDmg;
      totalDmg += dmg;
      let maxDmg =
        getAtkDmg(
          data,
  ...
  private async handleNormalCommands(msg: Message) {
    ...
    if (data.lastPlayedAt === nowTimeStr || data.lastPlayedAt === nextTimeStr) {
      if (
        msg.includes(
          Array.isArray(serifs.rpg.command.onemore)
        ...
        if (
          ...
          data.lastPlayedAt !==
            getDate() +
              (new Date().getHours() < 12 + (skillEffects.rpgTime ?? 0)
                ? ''
                : new Date().getHours() < 18 + (skillEffects.rpgTime ?? 0)
                  ? '/12'
                  : '/18')
        ) {
    ...
    if (
      ...
      data.lastPlayedAt === TimeStrBefore1
    ) {
      continuousBonus = 1;
    } else {
      if (
    ...
    continuousBonus =
      continuousBonus * (1 + (skillEffects.continuousBonusUp ?? 0));
    ...
    if (continuousBonus > 0) {
      continuousBonusNum =
        Math.min(Math.max(10, postCount / 2), 25) * continuousBonus;
    ...
    if (!data.enemy || count === 1) {
      ...
      if (filteredEnemys.length && !data.endressFlg) {
        /** 1度も倒した事のない敵 */
        const notClearedEnemys = filteredEnemys.filter(
          (x) => !(data.clearHistory ?? []).includes(x.name),
        );
        if (notClearedEnemys.length) {
          // 出現条件を満たしている敵の中で、1度も倒した事のない敵がいる場合、優先的に選ばれる
          data.enemy =
            notClearedEnemys[
              Math.floor(notClearedEnemys.length * Math.random())
            ];
        } else {
          // 1度も倒した事のない敵が誰もいない場合
          data.enemy =
      ...
      message += `$[x2 ${me}]\n\n${serifs.rpg.start}\n\n`;
      data.ehp =
        typeof data.enemy.maxhp === 'function'
          ? data.enemy.maxhp(100 + lv * 3)
          : Math.min(
              100 + lv * 3 + (data.winCount ?? 0) * 5,
    ...
    if (aggregateTokensEffects(data).showPostBonus) {
      ...
      if (continuousBonus >= 1) {
        ...
        message +=
          serifs.rpg.postBonusInfo.continuous.b(
            Math.floor(continuousBonusNum),
          ) + `\n`;
      } else if (continuousBonus > 0) {
        message +=
          serifs.rpg.postBonusInfo.continuous.c(
            Math.floor(continuousBonusNum),
          ) + `\n`;
      }
      if (isSuper) {
    ...
    let enemyAtk =
      typeof data.enemy.atk === 'function'
        ? data.enemy.atk(atk, def, spd)
        : lv * 3.5 * data.enemy.atk;
    /** 敵の防御力 */
    let enemyDef =
    ...
    for (let actionX = 0; actionX < plusActionX + 1; actionX++) {
      ...
      for (let i = 1; i < spd; i++) {
        if (
          data.enemy.abort &&
          Math.random() < data.enemy.abort * (1 - (skillEffects.abortDown ?? 0))
        ) {
          abort = i;
          break;
        }
      ...
      const defDmgX = Math.max(
        1 *
          ...
          (1 -
            Math.min(
              (skillEffects.tenacious ?? 0) * (1 - playerHpPercent),
              0.9,
            )),
        0,
      );
      ...
      let predictedDmg =
        Math.round(
          atk *
            ...
            (atkMinRnd + atkMaxRnd) *
            (1 /
              ((enemyDef * (getVal(data.enemy.defx, [tp]) ?? 3) + 100) / 100)),
        ) * (abort || spd);
      ...
      if (!data.enemy.spd && !data.enemy.hpmsg && !isTired) {
        ...
        if (
          ...
          if (
            ...
            if (data.enemy.fire && count > (data.thirdFire ?? 0))
      ...
      for (let i = 0; i < spd; i++) {
        ...
        // メッセージの出力
        message +=
          (crit ? `**${data.enemy.atkmsg(dmg)}**` : data.enemy.atkmsg(dmg)) +
          '\n';
        enemyHp -= dmg;
        if (dmg - noItemDmg > 1) {
      ...
      if (isSuper && enemyHp > 0 && isBattle && isPhysical) {
        message +=
          serifs.rpg.fireAtk(data.enemy.dname ?? data.enemy.name) + `\n`;
      ...
      if (enemyHp <= 0) {
        ...
        // ～を倒す系の記録
        if (data.enemy.name === ':aichan8:' && !data.aHeroLv) {
        ...
        if (
          ...
          message +=
            serifs.rpg.skill.ice(data.enemy.dname ?? data.enemy.name) + `\n`;
          enemyTurnFinished = true;
        ...
        if (
          ...
          message +=
            serifs.rpg.skill.light(data.enemy.dname ?? data.enemy.name) + `\n`;
        ...
        if (
          ...
          message +=
            serifs.rpg.skill.spdDown(data.enemy.dname ?? data.enemy.name) +
            `\n`;
          data.enemy.spd = 1;
        } else if (
          isBattle &&
          ...
          message +=
            serifs.rpg.skill.dark(data.enemy.dname ?? data.enemy.name, dmg) +
            `\n`;
          enemyHp -= dmg;
        } else if (!(isBattle && isPhysical)) {
        ...
        if (!enemyTurnFinished) {
          ...
          if (
            ...
            Math.random() < endure * (1 + (skillEffects.endureUp ?? 0))
          ) {
            message += serifs.rpg.endure + '\n';
            playerHp = 1;
            data.endure = Math.max(data.endure - 1, 0);
        ...
        if (playerHp <= 0) {
          ...
          if (
            skillEffects.escape &&
            ((data.escape ?? 0) < skillEffects.escape ||
              Math.random() <
                1 / 2 ** ((data.escape ?? 0) + 1 - skillEffects.escape))
          ) {
            // 逃走成功の場合
            message +=
              '\n' +
              (data.enemy.escapemsg ??
                (isBattle ? serifs.rpg.escape : serifs.rpg.escapeNotBattle));
          ...
          if (actionX === plusActionX) {
            message +=
              showStatus(data, playerHp, enemyHp, enemyMaxHp, me) +
              '\n\n' +
              serifs.rpg.next;
          } else {
            message +=
    ...
    const calcRerollOrbCount =
      Math.max(
        Math.min(
          Math.floor(
            (data.lv - skillBorders[1]) /
              ((skillBorders[2] - skillBorders[1]) / 5),
          ),
          5,
        ),
        0,
      ...
      Math.max(
        Math.min(
          Math.floor(
            (data.lv - skillBorders[2]) /
              ((skillBorders[3] - skillBorders[2]) / 10),
          ),
          10,
        ),
        0,
      ...
      Math.max(
        Math.min(
          Math.floor(
            (data.lv - skillBorders[3]) /
              ((skillBorders[4] - skillBorders[3]) / 17),
          ),
          17,
        ),
        0,
      ...
      Math.max(
        Math.floor((data.lv + 1 - skillBorders[4]) / (skillBorders[4] / 127.5)),
        0,
      ) +
      Math.max(
        Math.floor(
          (data.lv - (skillBorders[4] + 1)) / ((skillBorders[4] + 1) / 4),
        ),
        0,
      ) *
        16;
