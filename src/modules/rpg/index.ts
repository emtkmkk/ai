...
export default class extends Module {
		...
		setInterval(() => {
			...
			if (hours === 23 && new Date().getMinutes() >= 55 && new Date().getMinutes() < 60) {
				const friends = this.ai.friends
					.find()
					.filter((x) => x.perModulesData?.rpg?.lv && x.perModulesData.rpg.lv > 1 && x.perModulesData.rpg.noChart);
	...
	private async mentionHook(msg: Message) {
		...
		if (msg.includes([serifs.rpg.command.rpg]) && msg.includes([serifs.rpg.command.trial])) {
			...
			message += `\n${serifs.rpg.end}\n\n${serifs.rpg.trial.result(totalDmg)}\n${serifs.rpg.trial.random(
				Math.round(totalDmg * minRnd),
				Math.round(totalDmg * maxRnd)
		...
		if (msg.includes([serifs.rpg.command.rpg])) {
			...
			/** 1回～3回前の時間の文字列 */
			let TimeStrBefore1 =
				new Date().getHours() < 12 ? getDate(-1) + '/18' : new Date().getHours() < 18 ? getDate() : getDate() + '/12';
			let TimeStrBefore2 =
				new Date().getHours() < 12 ? getDate(-1) + '/12' : new Date().getHours() < 18 ? getDate(-1) + '/18' : getDate();
			let TimeStrBefore3 =
				new Date().getHours() < 12 ? getDate(-1) : new Date().getHours() < 18 ? getDate(-1) + '/12' : getDate(-1) + '/18';
			...
			if (data.lastPlayedAt === nowTimeStr) {
				...
				if (msg.includes([serifs.rpg.command.onemore])) {
					...
					if (
						...
						nowTimeStr =
							new Date().getHours() < 12 ? getDate() + '/12' : new Date().getHours() < 18 ? getDate() + '/18' : getDate(1);
					} else {
			...
			if (data.lastPlayedAt === nowTimeStr || data.lastPlayedAt === TimeStrBefore1) {
				...
				if ([TimeStrBefore2, TimeStrBefore3].includes(data.lastPlayedAt) || data.lastPlayedAt?.startsWith(getDate(-1))) {
			...
			/** 覚醒状態か？*/
			const isSuper =
				Math.random() < 0.02 + Math.max(data.superPoint / 200, 0) || (data.lv ?? 1) % 100 === 0 || color.alwaysSuper;
			...
			if (!data.enemy || count === 1) {
				...
				const filteredEnemys = enemys.filter(
					(x) => !(data.clearEnemy ?? []).includes(x.name) && (!x.limit || x.limit(data, msg.friend))
				);
				...
				data.ehp =
					typeof data.enemy.maxhp === 'function'
						...
						: Math.min(100 + lv * 3 + (data.winCount ?? 0) * 5, data.enemy.maxhp ?? 300);
			} else {
			...
			if (
				...
				enemyHpPercent - playerHpPercent >= 0.5 / (1 + (skillEffects.haisuiUp ?? 0))
			) {
				buff += 1;
			...
			if (skillEffects.enemyStatusBonus) {
				const enemyStrongs =
					(enemyAtk / (lv * 3.5)) * (data.enemy.atkx ?? 3) + (enemyDef / (lv * 3.5)) * (data.enemy.defx ?? 3);
			...
			for (let actionX = 0; actionX < plusActionX + 1; actionX++) {
				...
				let predictedDmg =
					Math.round(
						atk * tp * (atkMinRnd + atkMaxRnd) * (1 / ((enemyDef * (this.getVal(data.enemy.defx, [tp]) ?? 3) + 100) / 100))
					) * (abort || spd);
				...
				for (let i = 0; i < spd; i++) {
					const rng =
						(atkMinRnd + Math.random() * atkMaxRnd) *
						(1 + (skillEffects.atkDmgUp ?? 0)) *
						(skillEffects.thunder ? 1 + (skillEffects.thunder * ((i + 1) / spd)) / (spd === 1 ? 2 : spd === 2 ? 1.5 : 1) : 1);
					/** クリティカルかどうか */
					...
					if (playerHp <= 0) {
						...
						if (
							...
							((data.escape ?? 0) < skillEffects.escape ||
								Math.random() < 1 / 2 ** ((data.escape ?? 0) + 1 - skillEffects.escape))
						) {
							// 逃走成功の場合
	...
	private showStatus(
		...
		const PlayerHpInfoStr = data.enemy.pLToR
			? Math.ceil((100 - Math.min(Math.ceil(playerHp / (100 + (data.lv ?? 1) * 3) / (1 / 100)), 100)) / 5) * 5 +
			  ' ' +
			  serifs.rpg.infoPercent
			: Math.ceil(Math.min(Math.ceil(playerHp / (100 + (data.lv ?? 1) * 3) / (1 / 100)), 100) / 5) * 5 +
			  ' ' +
			  serifs.rpg.infoPercent;
		const playerHpStr = data.enemy.pLToR ? PlayerHpInfoStr : `${playerHp} / ${100 + (data.lv ?? 1) * 3}`;

		const debuff = [data.enemy.fire ? serifs.rpg.fire + data.count : ''].filter(Boolean).join(' ');

		if (data.enemy.pLToR) {
			return `\n${data.enemy.hpmsg ? serifs.rpg.player.hpmsg : me} : ${
				data.info && (data.clearHistory ?? []).includes(data.enemy.name) ? enemyHpInfoStr : enemyHpMarkStr
			}\n${data.enemy.hpmsg ?? data.enemy.dname ?? data.enemy.name} : ${data.info ? PlayerHpInfoStr : playerHpMarkStr}${
				debuff ? `\n${debuff}` : ''
			}`;
		} else {
			return `\n${data.enemy.hpmsg ?? data.enemy.dname ?? data.enemy.name} : ${
				data.info && (data.clearHistory ?? []).includes(data.enemy.name) ? enemyHpInfoStr : enemyHpMarkStr
			}\n${data.enemy.hpmsg ? serifs.rpg.player.hpmsg : me} : ${
				data.info >= 3 ? playerHpStr : data.info ? PlayerHpInfoStr : playerHpMarkStr
			}${debuff ? `\n${debuff}` : ''}`;
		}
	}
	...
	private getAtkDmg(
		...
		let dmg = Math.round(
			atk *
				...
				(1 / ((enemyDef * (defx ?? this.getVal(data.enemy.defx, [tp]) ?? 3) + 100) / 100))
		);
		if (data.fireAtk > 0) {
	...
	private getEnemyDmg(
		...
		let dmg = Math.round(
			enemyAtk *
				(this.getVal(data.enemy.atkx, [tp]) ?? 3) *
				(Math.max((count ?? 1) - 1, 1) * 0.5 + 0.5) *
				rng *
				(crit ? (typeof crit === 'number' ? 2 * crit : 2) : 1) *
				(1 / ((def * tp + 100) / 100))
