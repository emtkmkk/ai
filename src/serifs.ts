...
*/
	...
	rpg: {
		...
		trial: {
			...
			atk: (dmg) => `もこチキは木人に攻撃！\n${dmg}ポイントのダメージ！`,
			result: (totalDmg) => `合計${totalDmg.toLocaleString()}ポイントのダメージ！`,
			random: (down, up) => `(ダメージ幅: ${down.toLocaleString()} ~ ${up.toLocaleString()})`,
			best: (bestScore) => `(これまでのベスト: **${bestScore.toLocaleString()}**)`,
		},
		oneMore: {
			...
			autoBuy: (coin) => `おかわりRPGのプレイ権利を\n自動購入しました！\n（残りコイン数: ${coin.toLocaleString()}）`,
		},
		info: "もこチキの状況判断能力がアップ！\n今後、状況が細かく\n分析出来るようになる事があるぞ！",
		infoPercent: "%",
		postBonusInfo: {
			post: (num, bonus) => `投稿数 ${num.toLocaleString()} ステータス${bonus}%`,
			continuous: {
		...
		skills: {
			info: (num) => `スキルを変更する場合、\`rpg スキル変更 <数字>\`と話しかけてね (変更珠所持数: ${num.toLocaleString()})`,
			duplicationInfo: (num) => `スキルを変更&複製する場合、\`rpg スキル変更複製 <消すスキルの数字>\`と話しかけてね 消えたスキルの枠に既に持っているスキルのどれかが入るよ (複製珠所持数: ${num.toLocaleString()})`,
			list: "所持スキル一覧",
		},
		shop: {
		...
		help: {
			...
			okawari2: (num) => `[ RPG おかわり ]\n1日1回、このコマンドでRPGをもう一度遊べるみたいです！\nさらにもこコインを10枚使用すれば、さらにもう1回おかわりも出来ちゃうみたいです！\n過去にRPGをプレイし忘れた回数使えます！\n（あと${num}回使えるみたいですよ！）`,
