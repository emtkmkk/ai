...
*/
	...
	rpg: {
		...
		tired: (date, canOkawari) => `RPGモードは0~11時、12~17時、18~23時の1日3回です。\n${date.getHours() < 12 ? "12時以降" : date.getHours() < 18 ? "18時以降" : "明日"}になったらもう一度試してください。${canOkawari ? "\n（どうしてもプレイしたい場合は、「rpg おかわり」と話しかけてください）" : ""}`,
		start: "開始！",
		end: "終了！",
		turn: "ターン目",
		bonus: {
		...
		spdDownSkill: (enemyName) => `スキル「闇属性剣攻撃」発動！\n${enemyName}の行動速度が低下した！`,
		fireSkill: `スキル「炎属性剣攻撃」発動！\n攻撃時最低ダメージが上昇！`,
		iceSkill: (enemyName) => `スキル「氷属性剣攻撃」発動！\n${enemyName}は凍って動けなくなった！`,
		windSkill: (num) => `スキル「風属性剣攻撃」発動！\n行動回数+${num}！`,
		dartSkill: `スキル「土属性剣攻撃」発動！\n攻撃時最高ダメージが上昇！`,
		lightSkill: (enemyName) => `スキル「光属性剣攻撃」発動！\n${enemyName}の攻撃が弱体化した！`,
